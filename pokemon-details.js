...
async function loadPokemon(id) {
    try {
        ...
        .all([fetch(`https://pokeapi.co/api/v2/pokemon/${id}`).then((response) => response.json()),
...
function setElementStyles(elemements, cssProperty, value)
{
...
function setTypeBackgroundColor(pokemon)
{
    ...
    setElementStyles(document.querySelectorAll(".stats-wrap .stats"), "color", color);
...
function creatAndAppendElement(parent, tag, options = {})
{
...
function displayPokemonDetails(pokemon)
{
    ...
    types.forEach(({ type }) => {
        creatAndAppendElement(typeWrapper, "p", {
            className: `body3-fonts type ${type.name}`,
            textContent: type.name,
        });
    });

    document.querySelector(".pokemon-detail-wrap .weight").textContent = `${weight/10} KG`;
    document.querySelector(".pokemon-detail-wrap .height").textContent = `${height/10} metres`;

    const abilitiesWrapper = document.querySelector(".pokemon-detail-wrap .pokemon-detail.move");
    abilities.forEach(({ability}) => {
        creatAndAppendElement(abilitiesWrapper, "p", {
    ...
    stats.forEach(({stat, base_stat}) => {
        ...
        creatAndAppendElement(statDiv, "p", {
        ...
        creatAndAppendElement(statDiv, "progress", {
